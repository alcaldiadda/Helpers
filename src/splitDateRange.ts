...
export const splitDateRange = ({ start, end, timezone }: SplitDateRangeInput): Period[] => {
  ...
  while (startDate < endDate) {
    ...
    let endOfDay = startDate.set({ hour: 23, minute: 59, second: 59, millisecond: 999 });

    // If the end date is before the end of the current day, use the end date
    const endDayDuration = Interval.fromDateTimes(endOfDay, endDate).length("minutes");
    if (endDate < endOfDay || endDayDuration < 1) {
      endOfDay = endDate;
    }

    // Skip if the period duration is less than a minute
    const periodDuration = Interval.fromDateTimes(startDate, endOfDay).length("minutes");
