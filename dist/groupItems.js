...
var groupItems = function (items) {
    return items.reduce(function (acc, item) {
        if (item.parentId) {
            var parent_1 = items.find(function (p) { return p.$id === item.parentId; });
            if (parent_1) {
                var group = acc.find(function (g) { return g.group === parent_1.name; });
                if (group) {
            ...
            if (!acc.some(function (g) { return g.group === item.name; })) {
